...
async function main() {
  ...
  if (mode === MODE_MORNING || mode === MODE_BOTH) {
    await runTriage(MODE_MORNING);
  }

  if (mode === MODE_EVENING || mode === MODE_BOTH) {
    await runTriage(MODE_EVENING);
  }
}
...
async function runTriage(mode) {
  ...
  await writeDailyLog({ todayIso, ranked, top3, capacity, mode });
}
...
function buildSlackText({
  ...
  if (top3.length > 0) {
    addLine(lines, "🎯 TODAY — TOP 3", config.maxSlackLines);
...
function shouldRunThisHour(mode) {
  ...
  if (mode === MODE_MORNING) return localHour === config.morningHour;
  if (mode === MODE_EVENING) return localHour === config.eveningHour;
